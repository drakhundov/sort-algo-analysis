...
int medianOfMedians(const std::vector<int>& v, int k) {
  ...
  int findMedianOfMedians(std::vector<int> & v, int l, int r);
  void dutchNationalFlag(std::vector<int> & v, int& l, int& r, int p);
  int M, l, r, l_b, r_b;
  l = 0, r = v.size() - 1;
  while (l <= r) {
    M = findMedianOfMedians(v_cp, l, r);
    l_b = l, r_b = r;
    dutchNationalFlag(v_cp, l_b, r_b, M);
    if (k >= l_b && k <= r_b) {
      return M;
    }
    if (k < l_b) {
...
int findMedianOfMedians(std::vector<int>& v, int l, int r) {
  int n = r - l + 1;
  if (n <= 5) {
    std::vector cp(v.begin() + l, v.begin() + r + 1);
    bubbleSort(cp);
    return cp[n / 2];
  }
