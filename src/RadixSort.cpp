...
void radixSort(std::vector<int> &v, int ndigits) {
    void countingSortWithExp(std::vector<int> &, long);
    if (ndigits == -1) {
    ...
    int maxExp = (int) std::pow(10, ndigits - 1);
    for (long exp = 1; exp <= maxExp; exp *= 10) {
        countingSortWithExp(v, exp);
    }
}

void countingSortWithExp(std::vector<int> &v, long exp) {
